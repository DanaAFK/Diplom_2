...
class TestCreateUser:
    ...
    @allure.title('Cоздание пользователя с валидными данными')
    def test_create_user_valid_data(self, register_and_remove_user):
    ...
    @allure.title('Создания пользователя, который уже есть в базе данных')
    ...
    @allure.title('Ошибка при создании пользователя, если не заполнено обязательное поле password')
